...
- (void)createBar{
    ...
    if (self.selectedModel.count) {
        ...
        NSLog(@"%ld",self.selectedArray.count);
...
- (void)loadData{
    ...
    //selectedArray装着选择的cell

    self.selectedArray = [NSMutableArray arrayWithCapacity:0];
    
    //装着showIMGModel 的数组   存入PHFetchResult 和一个模型数组。目的就是判断PHasset 是否的select 值。并且返回一个装着shouIMGModel 数组
...
#pragma mark     ---- UICollection的代理方法。就不注解、、、、、
